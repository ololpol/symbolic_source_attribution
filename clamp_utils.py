...
class CLaMP(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        model = cls(config)
        model.load_state_dict(torch.load(pretrained_model_name_or_path+str('/pytorch_model.bin')), strict=False)

        return model

def compute_values(Q_e, K_e, softmax, t=1):
    """
...
def encoding_data(data, patchilizer, music_length):
    """
...
def load_music(filename = None, data = None):
    """
    ...
    if data == None:
        if filename != None:
            ...
            print("Either filename or raw data has to be provided")
    else: 
        output = data
...
def get_features(ids_list, model, device):
    """
